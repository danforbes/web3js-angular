...
import { type Address, Contract } from 'web3';

@Injectable({
  providedIn: 'root',
})
export class Erc20Service {
  private erc20: Contract<Erc20Abi>;

  constructor(web3Service: Web3Service) {
    this.erc20 = new web3Service.web3.eth.Contract(ERC_20.abi);
  }
  ...
  async deploy(
    ...
    const erc20Deployer = this.erc20.deploy({
      arguments: [initialAmount, tokenName, decimals, tokenSymbol],
      data: ERC_20.bytecode,
    });
    const deployTxn = await erc20Deployer.send({ from: deployerAddress });
...
type Erc20Abi = typeof ERC_20.abi;
